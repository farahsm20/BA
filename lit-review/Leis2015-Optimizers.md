...
> [cite_start] "The estimates of the other systems are worse and seem to be based on per-attribute histograms, which do not work well for many predicates and cannot detect (anti-)correlations between attributes" (Including PostgreSQL)
> [cite_start] "For PostgreSQL 16% of the estimates for 1 join are wrong by a factor of 10 or more. This percentage increases to 32% with 2 joins, and to 52% with 3 joins"
> [cite_start] "Given the simplicity of PostgreSQL’s join estimation formula and the fact that its estimates are nevertheless competitive with the commercial systems, we can deduce that the current join size estimators are based on the independence assumption."
> [cite_start] tbd Estimates for TPC-H 
